...
public class IntTreeNode {
     ...
     * @param data - data value to be added.
     */
    public IntTreeNode addAVLValue(int data) {
        if (data <= this.data) {
           if (left == null) {
               left = new IntTreeNode(data);
           } else {
               left = left.addAVLValue(data);
           }
        } else {
            if (right == null) {
                right = new IntTreeNode(data);
            } else {
                right = right.addAVLValue(data);
            }
        }
        // update height
        refreshHeight();
    ...
    /**
     * Rebalances the node using AVL methods
     */
    ...
    /**
     * Returns the balance factor of the class
     * Height of right - Height of left
     * @return
     */
    public int getBalanceFactor() {
        return (right != null ? right.height : 0) - (left != null ? left.height : 0);
        // int bf = left.height - right.height == 1 || left.height - right.height == 0 || left.height - right.height == -1 ? ":)" : ":("; 
    }
